...
### 保护体系
- **硬件短路**：PA6 BKIN 纳秒关断 + PA4 通知 MCU → 3 次重试 → 永久锁定
...
| PA2 | ADC1_IN3 | NTC 温度传感器 |
| PA3 | ADC1_IN4 | 母线电压采样 |
| PA4 | GPIO 输入 | 外部短路信号（比较器输出 → MCU 重试计数） |
| PA5 | BTN_UP | 菜单上/增加 |
...
| PB6 | USART1_TX | 调试串口 115200 |
| PB7 | USART1_RX | 调试串口 |
| PB8 | GPIO 输出 | 故障 LED（闪烁码） |
| PB9 | GPIO 输出 | 散热风扇 |
| PB10 | I2C2_SCL | SSD1306 OLED |
| PB11 | I2C2_SDA | SSD1306 OLED |

> **注意**：LQFP32 封装**没有** PB1 和 PB2！本项目已修正为 PB3/PB4。
...
| 保护 | 条件 | 动作 |
|---|---|---|
| 硬件短路 | PA6 BKIN 比较器触发 | 纳秒关 PWM + PA4 通知 MCU → 重试 3 次 → 锁定 |
...
- **短路必须走硬件**：外部比较器 → PA6 BKIN + PA4 MCU 通知。软件三级过流是补充，不能替代硬件。
